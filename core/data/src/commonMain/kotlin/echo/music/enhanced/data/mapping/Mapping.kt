package echo.music.enhanced.data.mapping
...
import echo.music.enhanced.domain.data.model.searchResult.songs.SongsResult
import echo.music.enhanced.domain.data.model.searchResult.songs.Thumbnail
import echo.music.enhanced.domain.data.model.searchResult.videos.VideosResult
...
import echo.music.enhanced.lyrics.models.response.LyricsResponse
import echo.music.enhanced.lyrics.models.response.TranslatedLyricsResponse
import echo.music.enhanced.lyrics.parser.parseRichSyncLyrics
import echo.music.enhanced.lyrics.parser.parseSyncedLyrics
import echo.music.enhanced.lyrics.parser.parseUnsyncedLyrics
...
internal fun Track.toSongItemForDownload(): SongItem =
    SongItem(
        ...
        artists =
            this.artists?.map {
                echo.music.enhanced.kotlinytmusicscraper.models.Artist(
                    id = it.id ?: "",
                    name = it.name,
                )
            } ?: emptyList(),
        album =
            echo.music.enhanced.kotlinytmusicscraper.models.Album(
                id = this.album?.id ?: "",
...
internal fun echo.music.enhanced.lyrics.domain.Lyrics.toLyrics(): Lyrics {
...
internal fun Lyrics.toLibraryLyrics(): echo.music.enhanced.lyrics.domain.Lyrics =
    echo.music.enhanced.lyrics.domain.Lyrics(
        lyrics =
            echo.music.enhanced.lyrics.domain.Lyrics.LyricsX(
                lines =
                    this.lines?.map {
                        echo.music.enhanced.lyrics.domain.Lyrics.LyricsX.Line(
                            endTimeMs = it.endTimeMs,
                            startTimeMs = it.startTimeMs,
                            syllables = listOf(),
                            words = it.words,
...
internal fun SearchSuggestions.toDomainSearchSuggestions(): echo.music.enhanced.domain.data.model.searchResult.SearchSuggestions =
    echo.music.enhanced.domain.data.model.searchResult.SearchSuggestions(
...
internal fun AccountInfo.toDomainAccountInfo(): echo.music.enhanced.domain.data.model.account.AccountInfo =
    echo.music.enhanced.domain.data.model.account.AccountInfo(
        name = this.name,
        email = this.email,
        pageId = this.pageId,
